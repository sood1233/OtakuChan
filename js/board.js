...
function postCardHtml(p, flash = false) {
  ...
  <div class="pc${flash ? ' flash' : ''}" id="post-${p.id}">
    ...
    <div class="acts">
      <button class="bl${liked.has(p.id) ? ' liked' : ''}" onclick="toggleLike('${p.id}', this)"><span class="ic">&hearts;</span> <span class="lc">${p.like_count}</span></button>
      <a class="br" href="thread.html?id=${p.id}"><span class="ic">&#128172;</span> ${p.reply_count}</a>
      <a class="br br-main" href="thread.html?id=${p.id}">View thread &rarr;</a>
      <span class="ai flag" onclick="openReport('${p.id}')">&#9873; Report</span>
    </div>
  </div>`;
}
...
async function loadTrending() {
  ...
  const [{ data: byLikes }, { data: byReplies }, { data: byViews }] = await Promise.all([
    sb.from('posts').select(POST_SELECT).eq('is_deleted', false).gte('created_at', since)
  ...
  document.getElementById('t-viewed').innerHTML = trendListHtml(byViews, 'view_count', '&#128065;');
}
...
function trendListHtml(list, field, icon) {
  ...
  return list.map(p => `
    <a class="tcard" href="thread.html?id=${p.id}">
      ...
      <div class="tmeta">${icon} ${fmtCount(p[field])}</div>
    </a>`).join('');
}
