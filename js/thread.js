...
async function loadThread() {
  ...
  const { data: p, error } = await sb.from('posts').select('*').eq('id', postId).eq('is_deleted', false).single();
  if (error || !p) {
  ...
  const { data: replies } = await sb.from('replies').select('*')
  ...
  wrap.innerHTML = `
    ...
    <div class="rfm">
      ...
      <input type="text" id="rf-name" placeholder="Anonymous" maxlength="50">
...
function replyHtml(r) {
  ...
  <div class="rc" id="reply-${r.id}">
    <div class="ph">
      <span class="nm">${esc(r.author || 'Anonymous')}</span>
...
async function submitReply() {
  ...
  try {
    ...
    nameEl.value = ''; bodyEl.value = ''; fileEl.value = '';
...
async function toggleLike(id, btn) {
  ...
  const { error } = await sb.from('likes').insert({ post_id: id, ip_hash: device });
...
function subscribeRealtime() {
  sb.channel(`thread-${postId}`)
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'replies', filter: `post_id=eq.${postId}` }, payload => {
      const list = document.getElementById('replies-list');
      const empty = list.querySelector('.no-t');
      if (empty) list.innerHTML = '';
      list.insertAdjacentHTML('beforeend', replyHtml(payload.new));
    })
